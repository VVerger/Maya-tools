...
class ToolboxPages(QWidget, FileManagment):
    ...
    def createButtonAsset(self, liste, path):
        ...
        #setdress
        if 'character' not in path and 'props' not in path:
            btnName = 'Set Dress'
            set_dressBTN = QPushButton(btnName)
            set_dressBTN.setStyleSheet(
                f"QPushButton {{"
                f"border : 1px solid black;"
                f"background-color: qlineargradient(x1: 0, y1: 1, x2: 0, y2: 0, stop: 0 #5c7962, stop: 1 #0b3915);"
                f"min-height: 20px;"
                f"}}"
                f"QPushButton:hover {{"
            ...
            set_dressBTN.mousePressEvent = lambda event, btnPath = path, button = set_dressBTN, project = None, name = btnName: self.handleMousePress(event, btnPath, button, project, name)
            fileLayout.addWidget(set_dressBTN)
        ...
        for i in liste :
            ...
            for idx, name in enumerate(button_names):
                EditPath = path + '/' + i + '/houdini/edit/' + name
    ...
    def on_single_click(self):
        if self._name == 'Set Dress' : 
            FileManagment.create_setDress(self, self._btnPath)
        # Méthode appelée pour le clic simple une fois le timer expiré
        else : 
            FileManagment.houdiniScene(self, self._btnPath, self._project, self._name)
